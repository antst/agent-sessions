...
func inspectGrokPermissionMode(socket string, pid int, record registryRecord) (string, bool) {
	if record.PID != pid || record.ProcStart == "" || processStart(pid) != record.ProcStart ||
	...
	if response.Type != "peer_inspection" || response.PID != pid || response.ProcStart != record.ProcStart ||
		response.SessionID != record.SessionID || response.Entrypoint != "grok" ||
		(response.PermissionMode != "default" && response.PermissionMode != "bypassPermissions") {
		return "", false
	}
