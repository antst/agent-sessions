...
func readClaudeTranscriptTitle(path, expectedSessionID string) (string, error) {
	...
	for scanner.Scan() {
		...
		if event.SessionID != expectedSessionID {
			return "", errors.New("claude transcript title identity does not match its catalog session")
		}
		title = strings.TrimSpace(event.CustomTitle)
		if title == "" || len(title) > 512 {
			return "", errors.New("claude transcript contains an invalid session title")
		}
	}
...
func QwenArtifactAttestationForPath(path string) (QwenArtifactAttestation, error) {
	identity, err := QwenArtifactIdentityForPath(path)
	if err != nil {
	...
	current, err := QwenArtifactIdentityForPath(path)
	if err != nil || current.Device != identity.Device || current.Inode != identity.Inode || current.Size != identity.Size {
		return QwenArtifactAttestation{}, errors.New("qwen artifact changed while capturing its content identity")
	}
	identity.Fingerprint = fingerprint
	return identity, nil
}

// QwenArtifactIdentityForPath captures only the stable filesystem identity of
// one mutable protocol file. Qwen appends to both protocol streams after
// launch, so restart reconciliation must not re-assert their initial bytes.
func QwenArtifactIdentityForPath(path string) (QwenArtifactAttestation, error) {
	info, err := os.Lstat(path)
	...
	return QwenArtifactAttestation{Path: path, Device: device, Inode: inode, Size: info.Size()}, nil
}
...
func QwenArtifactIdentityMatches(attestation QwenArtifactAttestation) bool {
	...
	device, inode, ok := durableFileIdentity(info)
	if !ok || device != attestation.Device || inode != attestation.Inode {
		return false
	}
	return attestation.Fingerprint == "" || QwenArtifactPrefixMatches(
		attestation.Path, attestation.Fingerprint, attestation.Size,
	)
}

// QwenArtifactPrefixMatches verifies that an append-only protocol file still
// begins with an earlier durable checkpoint. Device and inode alone are not
// sufficient because filesystems may immediately recycle an unlinked inode.
//
//nolint:gocyclo // Every file identity and bounded checkpoint condition is an independent fail-closed gate.
func QwenArtifactPrefixMatches(path, fingerprint string, size int64) bool {
	if !validSHA256Fingerprint(fingerprint) || size < 0 || size > 16*1024*1024 {
		return false
	}
	info, err := os.Lstat(path)
	...
	file, err := os.Open(path) //nolint:gosec // Exact bounded protocol path is lstat/fstat attested.
	if err != nil {
		return false
	}
	defer func() { _ = file.Close() }()
	...
	if err != nil || !os.SameFile(info, closed) || closed.Size() < size {
		return false
	}
	digest := sha256.Sum256(body)
	return fingerprint == "sha256:"+fmt.Sprintf("%x", digest[:])
}
