...
func TestStatusAndDoctorHaveFixedTruthfulSchema(t *testing.T) {
	...
	if status.Schema != Schema || status.Operation != "status" || !status.Ready || len(status.Checks) != 0 || len(status.Products) != 9 {
	...
	if doctor.Operation != "doctor" || len(doctor.Checks) != 11 || doctor.Checks[2].Status != "unavailable" {
