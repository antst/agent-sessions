...
func validateLaneOwner(persistent bool, pid int, procStart string) error {
	...
	if !exactProcessIdentityMatch(pid, procStart) {
		return errors.New("cannot corroborate a stable lifecycle owner; retry from a live Codex, Claude, or Antigravity session, or use --persistent")
	}
	return nil
}
...
func inferPeerParent(paths nativePaths, startPID int) (laneOwner, bool) {
	...
	if owner, ok := inferAgyParent(paths, startPID); ok {
		candidates = append(candidates, owner)
	}
	if len(candidates) == 0 {
		return laneOwner{}, false
	}
	// Nested agents inherit the outer product's environment. Prefer the live
	// owner deepest in the current process tree instead of relying on product
	// precedence. A future product adapter only needs to add another candidate.
	owner := candidates[0]
	for _, candidate := range candidates[1:] {
		if candidate.PID != owner.PID && processHasAncestor(candidate.PID, owner.PID) {
			owner = candidate
		}
	}
	return owner, true
