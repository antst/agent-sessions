...
var (
	nativeToolDefinitions = mustProductMCPTools("codex")
	claudeToolDefinitions = mustProductMCPTools("claude")
	grokToolDefinitions   = mustProductMCPTools("grok")
	qwenToolDefinitions   = mustProductMCPTools("qwen")

	mcpInstructions       = mustProductMCPInstructions("codex")
	claudeMCPInstructions = mustProductMCPInstructions("claude")
	grokMCPInstructions   = mustProductMCPInstructions("grok")
	qwenMCPInstructions   = mustProductMCPInstructions("qwen")
)

func mustProductMCPTools(product string) []map[string]any {
	definitions, err := sessiontools.ProductMCPTools(product)
	if err != nil {
		panic(err)
	}
	return definitions
...
func mustProductMCPInstructions(product string) string {
	instruction, err := sessiontools.ProductMCPInstructions(product)
	if err != nil {
		panic(err)
	}
	return instruction
...
// ProductMCPTools returns an isolated copy of the established product MCP
// inventory. The unified daemon owns execution, but the public schemas remain
// byte-for-byte derived from the working legacy integration.
func ProductMCPTools(product string) []map[string]any {
	definitions := mustProductMCPTools(product)
...
// ProductMCPInstructions returns the established product-specific guidance.
func ProductMCPInstructions(product string) string {
	return mustProductMCPInstructions(product)
}

// WrapPeerMessage preserves the native cross-session envelope used by the
// working per-product runtime while delivery moves behind the unified daemon.
...
func runMCPServer(toolDefinitions []map[string]any, instructions string, attest func(json.RawMessage) (string, error)) int {
	...
	if err := scanner.Err(); err != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions native MCP server failed: %v\n", err)
		return 1
	}
	return 0
}
...
func callNativePeerTool(name string, args map[string]any, callerSessionID string) (map[string]any, error) {
	...
	case "list_peers":
		...
		for _, peer := range result.Peers {
			productLabel, err := nativePeerProductLabel(peer.Entrypoint)
			if err != nil {
				return nil, err
			}
			lines = append(lines, fmt.Sprintf("%s — %s, %s, cwd %s, session %s, groups %s",
				defaultString(peer.DisplayName, peer.Name), productLabel,
...
func nativePeerProductLabel(entrypoint string) (string, error) {
	return sessiontools.ProductLabel(entrypoint)
}
...
func attestClaudeMCPCallerWithResolver(
	...
	if strings.TrimSpace(os.Getenv("AGENT_SESSIONS_PRODUCT")) != "claude" ||
		...
		return "", errors.New("claude launch context is unavailable")
	}
	parent, err := resolveParent(laneAgentRuntimeDir(), sessionID)
	...
	if !processHasAncestor(startPID, parent.AdapterPID) || !processHasAncestor(startPID, parent.PID) ||
		...
		return "", errors.New("claude MCP process ancestry does not match its grouped registration")
	}
...
func inferClaudeParentWithResolver(
	...
	// accept an ambient replacement UUID by itself.
