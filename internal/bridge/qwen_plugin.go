...
func verifyQwenPluginInstallation(root, expectedVersion string, enabled bool) error {
	...
	if !ok || stringValue(server["type"]) != "stdio" || stringValue(server["command"]) != qwenMCPCommand ||
		!argumentsOK || !validQwenMCPArguments(arguments) {
...
func validQwenMCPArguments(arguments []any) bool {
	if len(arguments) != 4 || stringValue(arguments[0]) != "connector" || stringValue(arguments[1]) != "qwen" ||
		stringValue(arguments[2]) != "--release-identity" {
		return false
	}
	identity := stringValue(arguments[3])
	return identity == "@AGENT_SESSIONS_RELEASE_ID@" ||
		len(identity) == 64 && strings.Trim(identity, "0123456789abcdefABCDEF") == ""
}
...
func runQwenPluginInstall(args []string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: %v\n", err)
		return 2
	}
	...
	if err := verifyQwenPluginInstallation(values.pluginRoot, values.version, true); err != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: source payload verification failed: %v\n", err)
		return 1
	}
	...
	if _, statErr := os.Lstat(root); statErr == nil {
		...
		if sameQwenPluginSource(source, values.pluginRoot) && !sameVersion {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: refuse non-rollbackable replacement: %v\n", err)
				return 1
			}
			uninstall := qwenPluginUninstallCommand(values.qwen, profile, os.Environ())
			uninstall.Stdin, uninstall.Stdout, uninstall.Stderr = os.Stdin, os.Stdout, os.Stderr
			if uninstallErr := uninstall.Run(); uninstallErr != nil {
				fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: remove prior selected-profile plugin: %v\n", uninstallErr)
				return 1
			}
			replacement = true
		}
	} else if !os.IsNotExist(statErr) {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: inspect installed Qwen plugin: %v\n", statErr)
		return 1
	}
	...
	if installErr := command.Run(); installErr != nil {
		if replacement {
			...
			if rollbackErr != nil {
				fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: native Qwen installer failed: %v; rollback failed: %v\n", installErr, rollbackErr)
				return 1
			}
			fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: native Qwen installer failed: %v; prior plugin restored\n", installErr)
			return 1
		}
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: native Qwen installer failed: %v\n", installErr)
		return 1
	}
	enabled, err := qwenPluginEnabled(statePath)
	if err == nil {
	...
	if err != nil {
		if replacement {
			rollbackErr := rollbackQwenPluginReplacement(values.qwen, root, rollbackSource, rollbackVersion, statePath, profile)
			if rollbackErr != nil {
				fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: post-install verification failed: %v; rollback failed: %v\n", err, rollbackErr)
				return 1
			}
			fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: post-install verification failed: %v; prior plugin restored\n", err)
			return 1
		}
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-install: post-install verification failed: %v\n", err)
		return 1
	}
	return 0
}
...
func runQwenPluginRemove(args []string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: %v\n", err)
		return 2
	}
	profile, err := qwenprofile.Current()
	if err != nil {
	...
	if stateErr != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: inspect selected-profile plugin state: %v\n", stateErr)
		return 1
	}
	if rootErr != nil && !errors.Is(rootErr, os.ErrNotExist) {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: inspect selected-profile plugin root: %v\n", rootErr)
		return 1
	}
	if err := refuseLiveQwenPluginMutation(profile); err != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: %v\n", err)
	...
	if err := command.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: native Qwen uninstaller failed: %v\n", err)
		return 1
	}
	if _, err := os.Lstat(root); !errors.Is(err, os.ErrNotExist) {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: post-remove plugin root remains: %v\n", err)
		return 1
	}
	if present, _, err := qwenPluginPolicy(statePath); err != nil || present {
		fmt.Fprintf(os.Stderr, "agent-sessions qwen-plugin-remove: post-remove policy remains or is unreadable: %v\n", err)
		return 1
	}
	return 0
}
