...
func TestQwenLaneUsageAdvertisesUnifiedSharedAndPermissionOptions(t *testing.T) {
	usage := qwenLaneUsage()
	for _, option := range []string{
		"-g, --group", "--inherit-groups", "--no-inherit-groups", "--qwen-home",
		"--yolo", "--no-yolo", "--approval-mode", "--persistent",
		"--auto-archive-after", "--no-auto-archive",
	} {
...
func TestQwenLaneTerminalNoticeIsExactAndDeduplicated(t *testing.T) {
	...
	wantedCollect := "qwen-peer-lane --host destination-host wait " + threadID
