...
func resumeClaudeLane(o claudeLaneOptions) (int, error) {
	...
	if state.Status != "archived" && managerIdentity.Status == processIdentityUnknown {
		return 1, errors.New("cannot corroborate the Claude lane manager from exact process identity; preserve it and retry")
	}
...
func validateClaudeLaneResumeState(state claudeLaneState) error {
	if state.Bare {
		return errors.New("claude lane state uses unsupported bare mode and cannot publish the required native peer socket; archive it and start a new lane")
	}
	if !state.WorkerSharedProfile {
		return errors.New("claude lane state lacks the required shared native profile; archive it and start a new lane")
	}
...
// ClaudeLaneReadiness is the non-secret authentication projection used by the
// unified daemon before it accepts a native Claude lane. The probe deliberately
// reuses the legacy shared-profile environment rules instead of inferring
// readiness from an installed executable.
...
// InspectClaudeLaneReadiness reports Claude's own authentication status for
// the same selected profile used by the legacy lane implementation.
func InspectClaudeLaneReadiness(claudeBin string) (ClaudeLaneReadiness, error) {
	status, err := inspectClaudeLaneAuthentication(claudeBin, resolveNativePaths())
...
func claudeLaneWorkerArgs(state claudeLaneState) []string {
	args := []string{
		...
		"--no-chrome",
		"--settings", `{"crossSessionInbound":"accept","disabledMcpjsonServers":["agent_sessions"]}`,
	}
	...
	managedTools := append([]string{"SendMessage", "ListAgents"}, claudeprofile.ManagedAgentSessionsMCPTools()...)
	allowedTools := strings.Join(managedTools, ",")
