...
func admitQwenDualOutput(path string, expected qwenAdmissionExpectation) (*qwenEventCursor, qwenSessionStart, error) {
	info, body, file, err := openAttestedQwenRegularFile(path, nil)
	if err != nil {
	...
	cursor := &qwenEventCursor{
		path: path, identity: info, file: file, offset: int64(len(consumed)), prefixDigest: sha256.Sum256(consumed),
	}
	file = nil
...
func (c *qwenEventCursor) ReadAvailable() ([]json.RawMessage, error) {
	if c == nil || c.identity == nil || c.file == nil {
		return nil, errors.New("qwen event cursor is not initialized")
	}
	opened, err := c.file.Stat()
	if err != nil || !opened.Mode().IsRegular() || !os.SameFile(c.identity, opened) {
		return nil, errors.New("qwen event cursor descriptor identity changed")
	}
...
// QwenNativeInput is the exact attested input transport used by the legacy
// Qwen host. It is exported as a narrow wrapper so the unified daemon can own
// the transport without copying or weakening its file-identity checks.
type QwenNativeInput struct{ writer *qwenInputWriter }

// OpenQwenNativeInput opens an existing daemon-owned Qwen input artifact.
func OpenQwenNativeInput(path string) (*QwenNativeInput, error) {
	writer, err := openQwenInputWriter(path)
	if err != nil {
		return nil, err
	}
	return &QwenNativeInput{writer: writer}, nil
}

// Submit appends one exactly re-attested native Qwen input frame.
func (input *QwenNativeInput) Submit(text string) error {
	if input == nil {
		return errors.New("qwen native input is unavailable")
	}
	return input.writer.Submit(text)
}

// Close releases the input descriptor without altering native history.
func (input *QwenNativeInput) Close() error {
	if input == nil || input.writer == nil || input.writer.file == nil {
		return nil
	}
	return input.writer.file.Close()
}
...
func readAttestedQwenRegularFile(path string, expected os.FileInfo) (os.FileInfo, []byte, error) {
	info, body, file, err := openAttestedQwenRegularFile(path, expected)
	if file != nil {
		_ = file.Close()
	}
...
//nolint:gocyclo // Explicit validation and lifecycle gates remain together for fail-closed auditability.
func openAttestedQwenRegularFile(path string, expected os.FileInfo) (os.FileInfo, []byte, *os.File, error) {
	...
	if expected != nil && !os.SameFile(expected, info) {
		return nil, nil, nil, errors.New("path identity changed")
	}
	if info.Size() > qwenDualOutputMaxBytes {
		return nil, nil, nil, errors.New("qwen protocol file exceeds the bounded size")
	}
	...
	if err != nil || !os.SameFile(info, opened) {
		_ = file.Close()
		return nil, nil, nil, errors.New("path changed while opening")
	}
	...
	if err != nil || !closed.Mode().IsRegular() || closed.Mode()&os.ModeSymlink != 0 || !os.SameFile(info, closed) || closed.Size() != int64(len(body)) {
		_ = file.Close()
		return nil, nil, nil, errors.New("path changed while reading")
	}
	return closed, body, file, nil
}
...
func observeQwenOwnedArtifacts(root string, paths []string) ([]qwenOwnedArtifact, error) {
	...
	for _, path := range paths {
		...
		if openErr != nil {
			closeObserved()
			return nil, fmt.Errorf("open qwen artifact %s: %w", path, openErr)
		}
		opened, openErr := file.Stat()
		if openErr != nil || !os.SameFile(info, opened) {
			_ = file.Close()
			closeObserved()
			return nil, fmt.Errorf("qwen artifact %s changed while opening", path)
		}
		seen[path] = true
		result = append(result, qwenOwnedArtifact{Path: path, identity: info, file: file})
	}
	return result, nil
}
