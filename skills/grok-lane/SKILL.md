...
grok-peer-lane --host HOST doctor --json
grok-peer-lane --host HOST list --all
```

Require a connected hub, destination capability `grok-lane`, and healthy contract version 2. Then
replace each `grok-peer-lane` command below with
`grok-peer-lane --host HOST`. Federation carries the attested parent context
and returns terminal notices through grouped routing; do not pass `--persistent`
or `--no-auto-archive`. Pass `-C /absolute/remote/path` when cwd matters. Remote stdin
...
Require `contract_version: 2`, `grok_available: true`, and no `grok_error`.
...
- `--persistent` disables parent-exit cleanup only. The normal terminal auto-archive grace still
...
- Always archive a retained lane when orchestration ends.
