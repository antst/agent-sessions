...
- verified: Grok Build 1.0.13 — an offline lane launches the ACP primary without `--resume`; `session/load {sessionId:<product-id>}` is the sole resume selector. Supplying the same id through both `--resume` and `session/load` stalled without an ACP response. source: `c5b280d:internal/bridge/grok_native_session.go:191-227`; `/home/antst/agentbus-evidence/grok-runtime-20260906T124120Z/cells/W2-resume-failure-wire.txt`
...
- verified: Grok Build 1.0.13 — an authenticated observer held across leader startup prevents the private leader from disappearing before the primary and resident observer attach. source: `c5b280d:internal/bridge/grok_native_session.go:83-135`; `c5b280d:internal/bridge/grok_native_observer.go:22-45`
...
- verified: Grok Build 1.0.13 — a live exact roster row maps `working` to busy, `needs_input` to waiting, and `idle` to idle; missing `resident` or `yolo`, duplicate exact rows, and unknown live activity are authority errors. source: `c5b280d:internal/bridge/grok_roster.go:58-116`
...
- verified: Grok Build 1.0.13 — peer mode uses the user's default Grok leader and starts only `grok agent --leader stdio`; it does not create a private leader. source: `/home/antst/agentbus-evidence/grok-20260906T072333Z/P8-product-mcp-raw-v5.txt`; `/home/antst/agentbus-evidence/grok-20260906T072333Z/P8-no-leader-raw-v5.txt`
- verified: Grok Build 1.0.13 — the managed interactive surface preserves native informational commands, requires an exact `--session-id` or valued `--resume`/`--load` identity, and rejects bare resume, `--continue`, `--fork-session`, and caller-owned leader selection because none identifies one attached session. source: `/home/antst/agentbus-evidence/grok-20260906T072333Z/P0-version-help.txt`; `c5b280d:internal/launcher/grok_peer.go:1-240`
