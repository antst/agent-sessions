...
- `CONTRADICTION:` The peer backend can replace an identity when one MCP helper observes a different `_meta.threadId`, but Codex 0.153.4 starts a separate helper after `/clear` and leaves the old helper connected; the observed product topology therefore does not exercise `Peer.Replace`. — source: `/home/antst/agentbus-evidence/codex-cells-20260906T094434Z/C8-peer-identity/mcp-process-details.txt`
