...
| L-25 | Grok normal archive and crash reconciliation remove the real ACP worker and all attributable MCP/tool descendants on Linux and macOS. The macOS restricted-executable ancestry path is exercised explicitly; unmanaged restricted daemons that both create a new session and reparent before observation are explicitly unsupported and excluded from a green cleanup claim, never guessed or killed heuristically. |
