...
| Ledger item | Decision | Source-backed reason |
| --- | --- | --- |
| Resident wrapper | With a launch token, `grok-peer` owns one private leader, one authenticated ACP primary, and one observer for the exact lane session. Without a token, it starts and owns one private leader, passes that socket to an inherited-terminal TUI child and one observer, and stays resident with one peer connection, one caller, and one private action endpoint. It propagates signals and the TUI's exit status, then joins the leader. Headless-only flags are rejected with an error naming lane mode; a hand-started Grok with no launcher identity never hellos. | Grok Build 1.0.13 launches its configured stdio MCP helper once per tool call, so that helper cannot own presence or caller state. A headless invocation auto-started a detached default leader which outlived the command, and that stale leader held later interactive startup at `Starting session`; the launcher-owned private leader makes process ownership explicit. |
...
| Tools | In lane mode the wrapper publishes `lanes/<launch-token-digest>.sock`; in peer mode it publishes an equally private digest-named socket. Grok invokes `grok-peer mcp` as a short-lived stdio helper, which sends one `{action, arguments}` request to that socket. The resident wrapper owns the caller state and, in peer mode, delivers through its private-leader observer. Without the exact live session in that leader's roster, delivery is rejected as `no_leader`. | ACP is the wrapper-to-Grok control protocol; MCP is the product-facing Agentbus tool boundary. The private paths disappear with the wrapper, and helper exit cannot lose a local turn registry. |
