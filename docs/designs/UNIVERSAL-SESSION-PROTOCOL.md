...
| Ledger item | Decision | Source-backed reason |
| --- | --- | --- |
| Resident wrapper | With a launch token, `grok-peer` owns one private leader, one authenticated ACP primary, and one observer for the exact lane session. Without a token, it starts and owns one private leader, authenticates one forever-quiet hold before starting the inherited-terminal TUI child, then opens one observer only after the exact session entry is ready; it stays resident with one peer connection, one caller, and one private action endpoint. It propagates signals and the TUI's exit status, then joins both ACP clients and the leader. Headless-only flags are rejected with an error naming lane mode; a hand-started Grok with no launcher identity never hellos. | Grok Build 1.0.13 launches its configured stdio MCP helper once per tool call, so that helper cannot own presence or caller state. Its private leader needs an authenticated client before the TUI starts, but that quiet connection cannot later list; the post-readiness observer owns roster and delivery. Ready means the exact session entry exists and either it was absent before TUI start or its mtime differs from the pre-start value. A headless invocation auto-started a detached default leader which outlived the command, and that stale leader held later interactive startup at `Starting session`; the launcher-owned private leader makes process ownership explicit. |
