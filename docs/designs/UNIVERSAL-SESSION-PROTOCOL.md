...
| Ledger item | Decision | Source-backed reason |
| --- | --- | --- |
| Resident wrapper | With a launch token, `grok-peer` owns one private leader, one authenticated ACP primary, and one observer for the exact lane session. Without a token, it starts and owns one private leader, authenticates one forever-quiet hold before starting the inherited-terminal TUI child, then opens one observer only after the exact session summary is ready; it stays resident with one peer connection, one caller, and one private action endpoint. It propagates signals and the TUI's exit status, then joins both ACP clients and the leader. Headless-only flags are rejected with an error naming lane mode; a hand-started Grok with no launcher identity never hellos. | Grok Build 1.0.13 launches its configured stdio MCP helper once per tool call, so that helper cannot own presence or caller state. Its private leader needs an authenticated client before the TUI starts, but that quiet connection cannot later list; the post-readiness observer owns roster and delivery. Ready means `<entry>/summary.json` exists and either it was absent before TUI start or its mtime differs from the pre-start value. The entry directory itself appears before the session is registered, while `summary.json` is committed atomically later. A headless invocation auto-started a detached default leader which outlived the command, and that stale leader held later interactive startup at `Starting session`; the launcher-owned private leader makes process ownership explicit. |
...
| Tools | In lane mode the wrapper publishes `lanes/<launch-token-digest>.sock`; in peer mode it publishes an equally private digest-named socket. Grok invokes `grok-peer mcp` as a short-lived stdio helper, which sends one `{action, arguments}` request to that socket. The resident wrapper owns the caller state and, in peer mode, delivers through the post-readiness observer; the quiet startup hold never serves tools or roster requests. Without the exact live session in that leader's roster, delivery is rejected as `no_leader`. | ACP is the wrapper-to-Grok control protocol; MCP is the product-facing Agentbus tool boundary. The private paths disappear with the wrapper, and helper exit cannot lose a local turn registry. |
...
| Size cap | **1,140 production / 1,175 test logical lines**, including ACP framing, both launcher modes, and leader bootstrap but excluding the shared wrapper host. | The measured implementation is 1,137 / 1,172: the interactive launcher owns the private leader, quiet hold, post-readiness observer, artifact gate, and exact headless classification in addition to the lane's ACP lifecycle. |
