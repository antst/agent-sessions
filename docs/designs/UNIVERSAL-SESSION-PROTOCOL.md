...
| --- | --- | --- |
| Process and connection | Lane mode and wrapped peer mode each start one resident wrapper. It owns the product session, the one daemon connection, one caller-kit instance, and one private Unix endpoint. A product-spawned stdio MCP helper is stateless and may exit after any call. | Caller `start`/`status`/`wait` state stays in the resident process rather than in a helper whose lifetime the product controls. The interactive child keeps its inherited terminal while the wrapper propagates signals and exit status. |
| Installed entry forms | One installed integration image is named `<native>-peer` and exposes two entry forms: invocation without a launch token owns the interactive product and its peer connection; invocation with a launch token holds the worker connection and owns a headless child. Both resident forms expose the same private action endpoint to `<native>-peer mcp`. | The environment is the mode discriminator for native and wrapped products alike; no product-specific worker flag enters the daemon, and the MCP helper owns no session state. |
...
| Tool ingress | The resident wrapper owns a private Unix socket under `dirname(AGENTBUS_SOCKET)/lanes/`, unlinks it on exit, and passes that path as `AGENTBUS_LANE_SOCKET` to the product-spawned `<product>-peer mcp` helper. The helper sends one `{action, arguments}` value to that endpoint and returns its one result. A product that cannot spawn a stdio helper may use another private endpoint, but none of Claude, Codex, Grok, or Qwen needs one. | Each mode has exactly one daemon connection and one caller-kit instance, both wrapper-owned. A per-session Unix path cannot be reused by a stray child as a different session's endpoint, and a short-lived helper never becomes presence or owns caller state. |
| Shared MCP entry | `wrappers/mcp` is one stdio MCP server for the caller-kit tool surface. Its private backend carries one action request to the resident wrapper; the wrapper applies it to its one caller-kit instance. Each product adds only `<product>-peer mcp` dispatch and its private socket configuration. A raw framed method hop remains test-only. | One MCP implementation and one stateless backend prevent every wrapper from rebuilding tool JSON or putting `start`/`status`/`wait` state in a process the product may kill. Budget: **200 production / 200 test logical lines**. |
...
| Shared deletion | Delete the 16 shared files in `internal/launcher`, including `lane_grok_test.go` exactly once (2,199 lines), and `cmd/agent-sessions/connector_refresh.go` plus its test (333 lines). Rewrite `connector.go`/test as the resident peer wrapper's connection, and rewrite `native_peer.go` as thin exec-time product configuration; the stdio entry is only the private action helper. | The old launcher package still dies: CLI parsing and thin peer plans move to `cmd`, product and connection ownership to wrappers, and lane recipes to wrappers. Connector self-exec/release refresh is unnecessary when the resident wrapper holds the connection. |
...
| Tools | In both modes `claude/.mcp.json` starts stateless `claude-peer mcp` calls against the resident wrapper's private Unix endpoint. The wrapper owns the peer or worker connection and its one caller. After Claude `/clear`, the wrapper observes the new product session and performs a different-ID re-hello before serving its actions. | The two resident forms share the caller kit but never coexist for one session; `/clear` ends the old transient peer identity instead of creating an `inactive` side state, and helper exit loses no caller state. |
...
| Tools | In both modes the resident wrapper supplies a private `codex-peer mcp` Unix endpoint in App Server `thread/start` `mcp_servers`; each stdio invocation is a stateless action helper. The wrapper defers its peer hello until the first thread identity exists. After Codex `/clear`, it observes the new thread ID and product title and performs a different-ID re-hello before serving that thread's actions. | MCP configuration is per thread; `/clear` removes the old transient identity and installs the new one without `inactive`, reconnect, host-global coordination, or helper-owned caller state. |
...
| Ledger item | Decision | Source-backed reason |
| --- | --- | --- |
| Resident wrapper | With a launch token, `grok-peer` owns one private leader, one authenticated ACP primary, and one observer for the exact lane session. Without a token, it starts `grok --leader --session-id <uuid>` as an inherited-terminal child and stays resident with one observer, one peer connection, one caller, and one private action endpoint. It propagates signals and the child's exit status. A hand-started Grok with no launcher identity never hellos. | Grok Build 1.0.13 launches its configured stdio MCP helper once per tool call, so that helper cannot own presence or caller state. The resident launcher reaches the product through Grok's default leader socket; that socket is a product-internal hop, not another bus connection. |
...
| Tools | In lane mode the wrapper publishes `lanes/<launch-token-digest>.sock`; in peer mode it publishes an equally private digest-named socket. Grok invokes `grok-peer mcp` as a short-lived stdio helper, which sends one `{action, arguments}` request to that socket. The resident wrapper owns the caller state and, in peer mode, delivers through the default leader observer. Without the exact live session in that leader's roster, delivery is rejected as `no_leader`. | ACP is the wrapper-to-Grok control protocol; MCP is the product-facing Agentbus tool boundary. The private path disappears with the wrapper, and helper exit cannot lose a local turn registry. |
