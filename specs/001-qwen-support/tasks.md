...
- [ ] T054 [US3] Execute all 16 live composition edges on Linux and record the contract plus cleanup evidence in `specs/001-qwen-support/evidence/us3-linux.md`
...
- [X] T070 [US5] Extend the foundational profile-scoped verifier with drift diagnostics plus exact live-upgrade/removal refusal, pre-proved rollback source, failed-replacement restoration, and reconciliation in `internal/bridge/qwen_plugin.go` and `internal/qwenprofile/profile.go`
...
- [ ] T078 [US5] Exercise default and explicit-profile upgrade/removal with live-session refusal plus a failed replacement that restores the prior enabled plugin, and record credential/settings/transcript nonmutation in `specs/001-qwen-support/evidence/us5-upgrade-remove.md`
...
- [X] T081C Record the product-neutral split-runtime/stale-shim upgrade finding and its transfer to a separate unified-user-runtime Spec Kit feature in `specs/001-qwen-support/spec.md` and `specs/001-qwen-support/evidence/runtime-lifecycle-debt.md`; do not fold the abandoned per-shim reconciliation design into this Qwen feature, and keep the release/tag gates blocked while permitting the reviewed Qwen boundary to merge into `develop`
