...
- [X] T053A [US3] Remove the hidden owner-plugin dependency from headless native composition: inject the current `agent_sessions` stdio MCP through one shared ACP server contract for Grok and Qwen session create/resume, add a fake-native regression that rejects an empty Grok MCP inventory or serialized capability environment, and preserve bounded JSON-RPC `error.data` plus the repeated non-deadline cause in private readiness diagnostics in `internal/bridge/acp_mcp.go`, `internal/bridge/grok_lane_manager.go`, `internal/bridge/qwen_lane_manager.go`, and their tests
