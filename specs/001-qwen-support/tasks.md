...
- [X] T016 Create the schema-valid `agent-sessions` Qwen plugin manifests, complete messaging skill, and minimal inventory/schema-valid shells for the four lane skills; route the MCP through an extension-rooted native entrypoint that receives a managed parent's exact runtime and never resolves a stale ambient `PATH` runtime; implement the foundational profile-scoped `install-qwen` target through Qwen's supported installer with exact post-install verification; and implement the sole Qwen-specific session-free readiness engine consumed later by launch admission, federation advertisement, and doctor in `qwen/plugin.json`, `qwen/scripts/native-entry`, `qwen/skills/agent-sessions/SKILL.md`, `qwen/skills/codex-lane/SKILL.md`, `qwen/skills/claude-lane/SKILL.md`, `qwen/skills/grok-lane/SKILL.md`, `qwen/skills/qwen-lane/SKILL.md`, `internal/bridge/qwen_plugin.go`, `internal/qwenprofile/profile.go`, `internal/qwenreadiness/readiness.go`, and `Makefile`; behavioral Qwen-parent lane guidance is owned exclusively by T052
...
- [X] T031 [US1] Using the foundational T016 selected-profile installer/verifier, execute the US1 runner against a real authenticated macOS Qwen installation and record exact elapsed time plus state/nonmutation evidence in `specs/001-qwen-support/evidence/us1-macos.md`
...
- [X] T045 [US2] Using the foundational T016 selected-profile installer/verifier, execute the full US2 lifecycle and crash runner on macOS and record exact ownership/archive evidence in `specs/001-qwen-support/evidence/us2-macos.md`
...
- [X] T053 [US3] Add a deterministic complete 16-edge composition runner with unique tokens, exact anchor assertions, one fresh trusted target workspace that contains no local MCP configuration, orderly SIGINT/SIGTERM cleanup, and synchronous exact-supervisor retirement in `scripts/test-qwen-composition` and `internal/bridge/supervisor.go`
- [X] T053A [US3] Remove the hidden owner-plugin dependency from headless native composition: inject the current `agent_sessions` stdio MCP through one shared ACP server contract for Grok and Qwen session create/resume, emit the required empty ACP environment array without serializing the launch capability, require the live matrix's isolated Grok profile and target workspace to contain neither an Agent Sessions plugin nor an `agent_sessions` MCP fallback, add a fake-native regression that rejects an empty or malformed Grok MCP inventory, and preserve bounded JSON-RPC `error.data` plus the repeated non-deadline cause in private readiness diagnostics in `internal/bridge/acp_mcp.go`, `internal/bridge/grok_lane_manager.go`, `internal/bridge/qwen_lane_manager.go`, `scripts/test-qwen-composition`, and their tests
...
- [X] T055 [US3] Execute all 16 live composition edges on macOS and record the contract plus cleanup evidence in `specs/001-qwen-support/evidence/us3-macos.md`
...
- [X] T063 [US4] Execute Linux-source to macOS-destination Qwen federation and record verbatim collection plus both-host cleanup evidence in `specs/001-qwen-support/evidence/us4-linux-to-macos.md`
- [X] T064 [US4] Execute macOS-source to Linux-destination Qwen federation and record verbatim collection plus both-host cleanup evidence in `specs/001-qwen-support/evidence/us4-macos-to-linux.md`
- [X] T065 [US4] Execute disconnected, unready, non-capable, and mixed-version remote negatives and record zero target creation/fallback in `specs/001-qwen-support/evidence/us4-negative.md`
...
- [X] T076 [US5] Perform a fresh no-Go prebuilt installation and peer/lane smoke on Linux and record hashes, profile preservation, and uninstall evidence in `specs/001-qwen-support/evidence/us5-linux.md`
- [X] T077 [US5] Perform a fresh no-Go prebuilt installation and peer/lane smoke on macOS and record hashes, profile preservation, and uninstall evidence in `specs/001-qwen-support/evidence/us5-macos.md`
- [X] T078 [US5] Exercise default and explicit-profile upgrade/removal with live-session refusal and record credential/settings/transcript nonmutation in `specs/001-qwen-support/evidence/us5-upgrade-remove.md`
...
- [X] T080A Project the real `peer-federator lane` flag and top-level usage product choices from the authoritative federator descriptor inventory, including Qwen, and regress the command surface in `cmd/peer-federator/main.go` and `cmd/peer-federator/main_test.go`
- [X] T080B Remove the stale three-product MCP/tool and Grok-adapter prose: keep the structured lane tool product-neutral, project its enum from the authoritative table, and document `fromProduct` validation as a descriptor projection in `internal/bridge/mcp.go`, `internal/bridge/product_test.go`, and `docs/GROK-ADAPTER.md`
