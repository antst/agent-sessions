...
func (p *fakeProduct) observed() [6]int32 {
	return [6]int32{p.calls[0].Load(), p.calls[1].Load(), p.calls[2].Load(), p.calls[3].Load(), p.calls[4].Load(), p.calls[5].Load()}
}

func startHarness(t *testing.T, p *fakeProduct, acknowledge bool) *rpc.Conn {
...
func runCase(t *testing.T, name string) [6]int32 {
	...
	case "terminal-before-interrupt":
		h := opened(t, p)
		must(t, h.Call(context.Background(), "turn.run", protocol.TurnRunRequest{SessionID: "product-session@local", Input: "ok"}, &TurnResult{}))
...
func checkDelivery(t *testing.T, h *rpc.Conn, disposition string) {
	...
	must(t, h.Call(context.Background(), "message.deliver", delivery, &receipt))
...
func opened(t *testing.T, p *fakeProduct) *rpc.Conn {
	h := startHarness(t, p, true)
	open(t, h)
	return h
}
