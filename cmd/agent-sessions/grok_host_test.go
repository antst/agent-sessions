...
func TestResolveGrokDaemonResumeUsesDetachedPeerNameAndRejectsLiveMatch(t *testing.T) {
	...
	snapshot.Catalog.Attachments[sessionID] = daemonpkg.ManagedAttachment{
		...
		NativeSessionID: sessionID, Name: "grok-v2-restart", Cwd: "/workspace",
	...
	snapshot.Catalog.Attachments["live-grok-id"] = daemonpkg.ManagedAttachment{
		...
		NativeSessionID: "live-grok-id", Name: "live-grok", Cwd: "/workspace", State: "preparing",
	}
	snapshot.Catalog.Attachments["other-cwd-id"] = daemonpkg.ManagedAttachment{
		...
		NativeSessionID: "other-cwd-id", Name: "grok-v2-restart", Cwd: "/other", State: "detached",
	}
	if _, err := runtime.State().Commit(snapshot.Revision, snapshot.Catalog); err != nil {
		t.Fatal(err)
	}

	selected, found, err := resolveGrokDaemonResume(runtime, launcher.GrokDaemonPrepareRequest{ResumeTarget: "grok-v2-restart", Cwd: "/workspace"})
	if err != nil || !found || selected.NativeSessionID != sessionID || selected.Name != "grok-v2-restart" {
		t.Fatalf("name resolution = %+v, found=%v, err=%v", selected, found, err)
	}
	...
	if _, _, err := resolveGrokDaemonResume(runtime, launcher.GrokDaemonPrepareRequest{ResumeTarget: "live-grok", Cwd: "/workspace"}); err == nil {
