...
`codex-peer-lane`: start a named Codex lane, collect its final answer, message or steer it while it
runs, resume the same transcript, and archive it.
...
Or drop `skills/codex-lane` into `~/.claude/skills/`. The Codex-side runtime must be installed
separately; `skills/codex-lane/references/install.md` carries the complete host setup and
...
- **No binary and no adapter code.** The only runtime dependency is `codex-peer-lane`; the protocol,
  wake, and portability logic stay in the Go runtime where fixes are centralized.
...
The skill targets adapter **contract version 2**, which requires a runtime providing `list`,
`doctor --json`, and `contract_version` in `lane.ready` and `doctor` output. Its `references/`
directory is the self-contained operational contract loaded from Claude's plugin cache. The source
repository also publishes the human-facing adapter specification under `docs/`.
